      ...
      for (const todo of todos) {
        if (!todo.id || !todo.content || !todo.status || !todo.priority) {
          return {
            success: false,
            error:
              "Each todo must have id, content, status, and priority fields",
          };
        }
  ...
  async updateTodoList(
    ...
    try {
      ...
      for (const update of updates) {
        ...
        if (
          update.priority &&
          !["high", "medium", "low"].includes(update.priority)
        ) {
          return {
            success: false,
        ...
        if (update.status) {
          todo.status = update.status as any;
        }
        if (update.content) {
          todo.content = update.content;
        }
        if (update.priority) {
          todo.priority = update.priority as any;
        }
